...
protected:
    ...
    /**
     * Table of transitions, indexed by the current state. Each byte in the table represents a set of 4 possible next states,
     * packed as 4 x 2-bit values: 8 bits DDCCBBAA, where AA are the low two bits, and AA = short space transition BB = short pulse transition
